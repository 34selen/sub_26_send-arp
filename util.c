...
int send_arp(const char *iface, const char *sender_ip_str, const char *target_ip_str, struct ether_addr *sender_mac)
{
    pcap_t *handle;
    char errbuf[PCAP_ERRBUF_SIZE];
    const char *device = iface;
    ...
    if (inet_pton(AF_INET, sender_ip_str, sender_ip) != 1)
    {
        fprintf(stderr, "Invalid IP address format: %s\n", sender_ip_str);
        return 1;
    }
